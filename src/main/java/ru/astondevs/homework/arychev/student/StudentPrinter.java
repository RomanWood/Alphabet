...
public class StudentPrinter {
    ...
    /**
     * Вывод в консоль, согласно заданию
     * @param students - не фильтрованный список
     * @param yearOfRelease - от года релиза книги
     * @param booksLimit - сколько книг
     */
    public static void printStudentBooks(List<Student> students,
                                  int yearOfRelease,
                                  int booksLimit) {
        ...
        students.stream()
                ...
                .map(Student::books)
                .flatMap(List::stream)
                .sorted(Comparator.comparingInt(Book::totalPages))
                .distinct()
                .filter(book -> book.releaseDate().getYear() >= yearOfRelease)
                .limit(booksLimit)
                .peek(System.out::println)
                .map(book -> book.releaseDate().getYear())
                .findFirst()
